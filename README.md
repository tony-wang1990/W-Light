...
| 自动化测试与发布质量 | 进行中 | 后端 11 个测试套件、42 个用例；已覆盖核心业务、HTTP 冒烟、路由映射、数据库迁移 SQL 和完整 App HTTP e2e；GitHub Actions CI 已接入常规质量检查和 PostgreSQL e2e，lint 剩余 14 个 reports 类型 warning，仍需真机验收 |
